...
runAlex :: Text -> Text -> Alex a -> Either SliceError a
...
newtype Alex a = Alex { unAlex :: AlexState -> Either SliceError (AlexState, a) }
...
alexError message = do
    pos <- getCurrentPos
    Alex . const . Left $ SliceError (Just pos) message
